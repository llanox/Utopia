...
public class FirstRunUtopiaServlet implements ServletContextListener {
	...
	private static Logger logger = Logger.getLogger(FirstRunUtopiaServlet.class);
	...
	ServletContext context = event.getServletContext();
		...
		String fileName = context.getInitParameter(ConstantsLudens.KEY_CONFIGURATION_FILE_NAME);
	   ...
	   //Up one level in directory structure
	    filePath = file.getParentFile().getAbsolutePath()+LudensUtilBean.getFileSeparator()+fileName;
        ...
        if(!file.exists()){
        	...
        	FirstRunConfiguration config = (FirstRunConfiguration) ctx.getBean(ConstantsLudens.FIRST_RUN_CONFIG_BEAN);
