...
public class EventProcessImpl implements EventProcess, UtopiaTask {
	...
	public void eventHappening() {
		...
		for(Player player : players){			
			...
			if(uevt.isGoodEvent()){				
				event = new MessageEvent(this,UtopiaUtil.getId(),EnumMsgs.POSITIVE_UNEXPECTED_EVENT,uevt.getMessage(),elementQty,result);				
			}else{			
				event = new MessageEvent(this,UtopiaUtil.getId(),EnumMsgs.NEGATIVE_UNEXPECTED_EVENT,uevt.getMessage(),elementQty,result);				
			}
