...
public class GameProcessImpl implements GameProcess {
	...
	public boolean meetReqToGettingStart(Player player) {
		...
		Set<Element> materials = elementService.getAllElementsByPlayer(
				EnumElementType.MATERIAL, player.getUser().getLogin());
		elements.addAll(materials);

		logger.debug("Materials " + materials.size());

		Set<Element> factors = elementService.getAllElementsByPlayer(
				EnumElementType.FACTOR, player.getUser().getLogin());
	...
	public void createAdditionalProcesses() {
		if (game != null) {
			...
			for (Player player : players) {
				...
				logger.debug("Creating processes by player "
						+ player.getUser().getLogin());

				ElementProcess elementProcess = serviceLocator
