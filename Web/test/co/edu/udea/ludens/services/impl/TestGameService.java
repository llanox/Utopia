...
public class TestGameService {
	...
	public void atLeastTwoPlayers(){
		...
		when(playerDao.findAllPlayersByGameName(true,game.getName())).thenReturn(game.getPlayers());
...
//		game.getDefaultIncrementables().add(factor);
	...
	public void generateTwoEventsByFactorOrMaterial(){
		...
		Assert.assertEquals("No se han generado 2 eventos por factor o material",4,game.getUnexpectedEvents().size());
	...
	public void generateUnexpectedEventBetweenRange(){
		...
		gameService.generateUnexpectedEvents(game);
		Assert.assertNotNull("No puede ser nulo el evento retornado", game.getUnexpectedEvents().get(0));
		int quantity = game.getUnexpectedEvents().get(0).getQuantity();
		Assert.assertTrue("Los porcentajes de afectación no estan en el rango permitido: "+quantity, quantity>=ConstantsLudens.LOWER_THRESHOLD && quantity<=ConstantsLudens.UPPER_THRESHOLD );
