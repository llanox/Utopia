...
public class TestAppCtx {
	...
	public  void preparing(){
		...
		systemContainer= appContext.getBean("systemContainer",SystemContainer.class);
		gameService = appContext.getBean("gameService",GameService.class);
		userService = appContext.getBean("userService",UserService.class);
		playerService = appContext.getBean("playerService",PlayerService.class);
...
//		Assert.assertEquals("NO modifico el juego", true, startsWith);
		...
		Assert.assertEquals("NO modifico el juego", EnumGameStatus.STARTED, game.getStatus());
