...
public class GameServiceImpl implements GameService {
	...
	private GameRequirement[]gameRequirements ={
			new AtLeastNPlayers(),
			new AtLeastNMaterial(),
			new AtLeastNFactors(),
    ...
    public static final int AT_LEAST_N_PLAYERS =2;
		...
		public void verify(Game game) {
			List<Player> players = playerDao.findAllPlayersByGameName(true,game.getName());

			if(players==null)
			throw new InvalidNumberOfPlayersException("Se debe tener al menos "+AT_LEAST_N_PLAYERS+" jugadores matriculados en el juego. Se tiene en cambio "+0+" jugadores");
			
			if(players.isEmpty() || players.size() < AT_LEAST_N_PLAYERS){
				logger.debug("Jugadores "+players.size());
				throw new InvalidNumberOfPlayersException("Se debe tener al menos "+AT_LEAST_N_PLAYERS+" jugadores matriculados en el juego. Se tiene en cambio "+players.size()+" jugadores");
			}
		}
    ...
    public class AtLeastNFactors implements GameRequirement{
        public static final int ALEAST_N_FACTORS =1;
    		...
    		public void verify(Game game) {
    			...
    			if(incrementables == null  ){
    				throw new InvalidNumberOfFactorsException("No hay factores definidos");
    			}
    			...
    			if(counter < ALEAST_N_FACTORS){
    				throw new InvalidNumberOfFactorsException("Se debe tener al menos "+ALEAST_N_FACTORS+" factores definidos en el juego");
    			}
    		}
	...
	public void generateUnexpectedEvents(Game game) {
		...
		for(Incrementable incr : incrementables){
			...
			logger.debug("Generating unexpected event for incrementable... "+incr.getName());
			...
			if(incr.getType()==EnumElementType.FACTOR || incr.getType() == EnumElementType.MATERIAL){
				...
				int quantity = UtopiaUtil.generateNumberBetweenRange(ConstantsLudens.LOWER_THRESHOLD, ConstantsLudens.UPPER_THRESHOLD);
				good.setQuantity(quantity);
				good.setMessage("Evento Fortuito: "+incr.getName()+" ha sido afectado positivamente en un "+good.getQuantity()+ " % ");				
				logger.debug("Evento Fortuito: "+incr.getName()+" ha sido afectado positivamente en un "+good.getQuantity()+ " % ");	
				
				bad.setElementName(incr.getName());
				logger.debug("Evento Fortuito: "+incr.getName()+" ha sido afectado negativamente en un "+good.getQuantity()+ " % ");
				bad.setMessage("Evento Fortuito: "+incr.getName()+" ha sido afectado negativamente en un "+good.getQuantity()+ " % ");				
				quantity = UtopiaUtil.generateNumberBetweenRange(ConstantsLudens.LOWER_THRESHOLD, ConstantsLudens.UPPER_THRESHOLD);
