...
public class TradeProcessImpl implements TradeProcess {
	...
	public void postOffer(String elFromSender, int quantityFromSender,	String elFromReceiver, int quantityFromReceiver, String offerSender) {
		...
		Player sender = UtopiaUtil.getPlayerByName(game.getPlayers(), offerSender);
	...
	MessageEvent event = new MessageEvent(this,UtopiaUtil.getId(),EnumMsgs.NEW_OFFER,sender);
	notifyMsg(event);
	...
	private boolean verifyAvailability(Element element, Integer quantityTosend) {
		...
		if(inventory.compareTo(quantityTosend) < 0){
			...
			MessageEvent event = new MessageEvent(this, UtopiaUtil.getId(),EnumMsgs.NO_AVAILABILITY, element.getIncrementable().getName());
			notifyMsg(event);
			availability = false;
		}
	...
	public void acceptOffer(Interchange interchange, String playerUser){
		...
		Player player =  UtopiaUtil.getPlayerByName(game.getPlayers(), playerUser);
	...
	public void acceptOffer(Interchange interchange, Player playerWhoAccept){
		...
		String nameElGivenByReceiver = interchange.getElFromReceiver().getIncrementable().getName();
		String nameElGivenBySender = interchange.getElFromSender().getIncrementable().getName();
		...
		MessageEvent event = new MessageEvent(this,UtopiaUtil.getId(), EnumMsgs.ACCEPTED_OFFER,playerWhoOffer.getUser().getLogin(),quantityFromSender,rdFromSender.getIncrementable().getName(),quantityFromReceiver,rdFromReceiver.getIncrementable().getName(),playerWhoAccept.getUser().getLogin());
	...
	public void cancelOffer(Interchange interchange) {
		...
		id_counter++;
		String sender = interchange.getSender().getUser().getLogin();
		MessageEvent event = new MessageEvent(this,UtopiaUtil.getId(), EnumMsgs.CANCEL_OFFER,sender);
		notifyMsg(event);
